...
bool Node::isIgnoreFile() const
{
    ...
    return client->ignoreFilesEnabled
           ...
           && name_it->second == Sync::IGNOREFILE_NAME;
}
...
void LocalNode::init(Sync* csync, nodetype_t ctype, LocalNode* cparent, string* cfullpath, std::unique_ptr<string> shortname)
{
    ...
    else
    {
        ...
        mIgnoreFilePath.append(sync->client->fsaccess->localseparator);
        mIgnoreFilePath.append(Sync::IGNOREFILE_NAME);
    }
...
LocalNode::~LocalNode()
{
    ...
    if (node)
    {
        ...
        else if (!(mExcluded || mParentLoadFailed))
        {
...
bool LocalNode::excluded(const string& name) const
{
    assert(type == FOLDERNODE);
    ...
    for ( ; node; node = node->parent)
    {
        ...
        localPath.insert(0, node->localname);
...
const string& LocalNode::ignoreFilePath() const
{
    assert(type == FOLDERNODE);
...
bool LocalNode::included(const string& name) const
{
    assert(type == FOLDERNODE);
...
void LocalNode::purgeFilterState()
{
    ...
    for (auto& child_it : children)
    {
        ...
        child.mExcluded = excluded(child.name);
    ...
    while (pending.size())
    {
        ...
        for(auto& child_it : node.children)
        {
            ...
            // Child might be excluded by global filters.
            child.mExcluded =
              child.parent->excluded(child.name);
...
{
    return children.count(&Sync::IGNOREFILE_NAME);
}
...
void LocalNode::restoreFileFilterState()
{
    ...
    mIsIgnoreFile = name == Sync::IGNOREFILE_NAME;
}
...
void LocalNode::updateFilterState(LocalNode* newParent, string* newPath)
{
    if (!newParent)
    {
        if (!isIgnoreFile())
    ...
    string oldName;
    string newName = fsAccess.getname(newPath);
    ...
    if (localname != newName)
    {
        ...
        else
        {
            ...
            mIgnoreFilePath.append(fsAccess.localseparator);
            mIgnoreFilePath.append(Sync::IGNOREFILE_NAME);
        }
...
bool LocalNode::updateGeneralFilterState()
{
    ...
    if (parent)
    {
        changed |= excluded(parent->excluded(name));
