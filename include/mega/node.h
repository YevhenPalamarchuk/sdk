...
struct MEGA_API LocalNode : public File
{
    ...
    bool excluded(const string& name) const;
    ...
    const string& ignoreFilePath() const;

    // Is name included by our or our parent's filter rules?
    bool included(const string& name) const;
...
private:
    ...
    void updateFilterState(LocalNode* newParent, string* newPath);
